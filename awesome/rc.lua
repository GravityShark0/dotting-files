...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey, "Shift" }, "q", function () awful.spawn(terminal.." -e joshuto") end,
              {description = "open a terminal with a file manager", group = "launcher"}),

    awful.key({ modkey, "Control" }, "r", awesome.restart,
              {description = "reload awesome", group = "awesome"}),
    awful.key({ modkey,   "Shift" }, "r", function () awful.spawn("/home/gravityshark/.xprofile") end,
              {description = "refresh profile", group = "awesome"})  ,
...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey, "Shift" }, ".", function (c) c:move_to_screen(-1); awful.screen.focus_relative(1)  end,
